...
esp_err_t bsp_extra_codec_volume_set(int volume, int *volume_set)
{
    ...
    if (volume < 0) {
        volume = 0;
    } else if (volume > CODEC_VOLUME_MAX) {
        ESP_LOGW(TAG, "volume %d capped to %d (CODEC_VOLUME_MAX)", volume, CODEC_VOLUME_MAX);
        volume = CODEC_VOLUME_MAX;
    }
